...
try {
    ...
    if ($selected_conversation_id) {
        ...
        if ($selected_conversation) {
            ...
            $user_query = "SELECT user_id, full_name, profile_img, role FROM users WHERE user_id = :user_id";
            $user_stmt = $db->prepare($user_query);
            $user_stmt->execute([':user_id' => $other_id]);
            $other_user = $user_stmt->fetch(PDO::FETCH_ASSOC);

            if ($other_user && !in_array($other_user['role'], ['admin', 'landowner'], true)) {
                $selected_conversation = null;
                $other_user = null;
            } else {
...
function getProfileImage($profileImg) {
    if ($profileImg && file_exists(__DIR__ . '/../../../assets/uploads/profiles/' . $profileImg)) {
...
<head>
    ...
    <title>Messages - DENR Verifier</title>
            ...
            <div class="dashboard-content admin-dashboard admin-messages">
                <header class="admin-dashboard-header">
                    <div>
                        ...
                        <p class="admin-dashboard-subtitle">Message administrators and landowners</p>
                    </div>
                </header>
                ...
                <div class="msg-app <?php echo ($selected_conversation && $other_user) ? 'msg-app--thread-open' : ''; ?>" role="application" aria-label="Messaging">
                            ...
                            <div class="msg-conv-pane" id="conversations-pane">
                                <ul class="msg-conv-list" aria-label="Users list">
                                    ...
                                    <li class="msg-empty-state">
                                        ...
                                        <p class="msg-empty-title">No contacts</p>
                                        <p class="msg-empty-desc">No administrators or landowners are available to message.</p>
                                    </li>
                                    <?php else: ?>
                    ...
                    <section class="msg-thread" aria-label="Conversation">
                        ...
                        <div class="msg-welcome" aria-label="No conversation selected">
                            ...
                            <p class="msg-welcome-desc">Choose an admin or landowner from the list to view or start a conversation.</p>
                        </div>
                        <?php endif; ?>
                    </section>
                </div>
