...
try {
    // Build users_list (same shape as admin): conversations + admin to start new
    $user_id = $_SESSION['user_id'];

    $conv_query = "SELECT c.conversation_id,
                    CASE WHEN c.user_1_id = :user_id THEN c.user_2_id ELSE c.user_1_id END as other_user_id,
    ...
    $conv_stmt->execute([':user_id' => $user_id]);
    $conversations = $conv_stmt->fetchAll(PDO::FETCH_ASSOC);

    $admin_query = "SELECT user_id, full_name, profile_img, email FROM users WHERE role = 'admin' LIMIT 1";
    ...
    foreach ($conversations as $c) {
        $u_stmt = $db->prepare("SELECT user_id, full_name, profile_img, email FROM users WHERE user_id = :id");
        $u_stmt->execute([':id' => $c['other_user_id']]);
        ...
        if ($u) {
            ...
            if ($admin_user && (int)$u['user_id'] === (int)$admin_user['user_id']) {
                $admin_in_list = true;
            }
        }
    }
    if ($admin_user && !$admin_in_list) {
        $users_list[] = [
            'user_id' => $admin_user['user_id'],
            'full_name' => $admin_user['full_name'],
            'profile_img' => $admin_user['profile_img'] ?? null,
    ...
    $verifier_stmt = $db->query("SELECT user_id, full_name, profile_img, email FROM users WHERE role = 'verifier' ORDER BY full_name");
    $verifier_rows = $verifier_stmt ? $verifier_stmt->fetchAll(PDO::FETCH_ASSOC) : [];
    foreach ($verifier_rows as $ver) {
        $vid = (int) $ver['user_id'];
        $already = false;
        foreach ($users_list as $entry) {
            if ((int) ($entry['user_id'] ?? 0) === $vid) {
                $already = true;
                break;
            }
        }
        if (!$already) {
            $users_list[] = [
                ...
                'profile_img' => $ver['profile_img'] ?? null,
                'email' => $ver['email'] ?? '',
                'conversation_id' => null,
                'last_message' => null,
                'last_message_time' => null,
    ...
    usort($users_list, function ($a, $b) {
        $t1 = $a['last_message_time'] ? strtotime($a['last_message_time']) : 0;
        $t2 = $b['last_message_time'] ? strtotime($b['last_message_time']) : 0;
...
function getProfileImage($profileImg) {
    $path = __DIR__ . '/../../../assets/uploads/profiles/' . ($profileImg ?? '');
    if ($profileImg && file_exists($path)) {
            ...
            <div class="dashboard-content admin-dashboard admin-messages">
                <header class="admin-dashboard-header">
                    <div>
                        ...
                        <p class="admin-dashboard-subtitle">Message DENR Admin and verifiers</p>
                    </div>
                </header>
                    ...
                    <aside class="msg-sidebar">
                        ...
                        <div class="msg-tab-content">
                            <div class="msg-conv-pane" id="conversations-pane">
                                <ul class="msg-conv-list" aria-label="Conversations list">
                                    <?php if (empty($users_list)): ?>
                                    <li class="msg-empty-state">
                                        <div class="msg-empty-icon"><i class="fas fa-comments"></i></div>
                                        <p class="msg-empty-title">No conversations</p>
                                        <p class="msg-empty-desc">Message an administrator or verifier to get started.</p>
                                    </li>
                                    <?php else: ?>
